...
| ``scrollbarGutter``      | ``scrollbar-gutter``        | When true, reserves space for the scrollbar using ``scrollbar-gutter: stable`` while the dialog is open.&nbsp;This prevents layout shifts when the page scrollbar disappears behind the modal backdrop.&nbsp;Disable when the modal covers the entire viewport (e.g. login) to avoid a visible gutter gap. | &#x2718; | ``boolean``                                                                          | ``true``                     |
