...
|   `validation-icon`    |  The icon to use with the validation message<ol><li> <b>URL</b>: (ex: /path/to/image.png)<li> <b>Data URL</b>: (ex: data:image/jpeg;base64,/9j/4SDpRXhpZgAAT....)<li> <b>ICON_NAME</b>: An icon to load from the default Dwc icons pool. (ex: ``x``)<li> <b>POOL_NAME:ICON_NAME</b>: An icon to load from the passed pool. (ex: ``feather:x``)</ol>       |    `string`   |   `'bbj:info'`    |
