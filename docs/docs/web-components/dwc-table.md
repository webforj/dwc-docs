...
| ``columnGroups``            |                               | An array of column group definitions for multi-row header grouping.&nbsp;When set, columns are ordered according to the group tree and a multi-row&nbsp;header is rendered with colspan/rowspan.&nbsp;&nbsp;When empty (default), behavior is identical to the standard single-row header.                                                                                                                                                                                                                                                                                                                                                   | &#x2718; | ``TableColumnGroupDefinition[]``        | ``[]``         |
...
| ``groupHeaderHeight``       | ``group-header-height``       | The height of group header rows in pixels. When not set, defaults to headerHeight.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | &#x2718; | ``number``                              |                |
...
| ``overscan``                | ``overscan``                  | The number of rows to pre render beyond the visible scrolling area.&nbsp;The bigger the number the slower the performance. keep this value small                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | &#x2718; | ``number``                              | ``10``         |
...
| ``scrollSkeletonMode``      | ``scroll-skeleton-mode``      | Controls whether skeleton placeholders are shown in cells with renderers&nbsp;during scroll instead of rendering the actual cell content.&nbsp;&nbsp;This improves scroll performance when cells contain web components&nbsp;(e.g. badges, progress bars, icons) by avoiding expensive shadow DOM&nbsp;creation on every scroll frame. The actual renderers materialize once&nbsp;scrolling stops.&nbsp;&nbsp;- ``auto`` - Show skeletons during scroll only when columns have cell renderers.&nbsp;- ``enabled`` - Always show skeletons during scroll.&nbsp;- ``disabled`` - Never show skeletons; always render cell content immediately. | &#x2718; | ``"auto" \| "disabled" \| "enabled"``   | ``'disabled'`` |
